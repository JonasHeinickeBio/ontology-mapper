...
class DemoConceptLookup:
    ...
    def lookup_concept(self, concept: Dict):
        ...
        comparison = {
            ...
            'discrepancies': ["Demo: Result count differs between services"] if len(bp_results) != len(ols_results) else []
        }
...
class DemoBioPortalGUI:
    ...
    def setup_demo_ui(self):
        """Setup demo UI"""
        # Header
        header = ttk.Label(self.root, text="🧪 BioPortal Ontology Alignment GUI - DEMO MODE", 
                          font=("Arial", 16, "bold"))
        header.pack(pady=10)
...
For full functionality, use the complete version with bioportal_cli.py."""
        ...
        ttk.Entry(file_entry_frame, textvariable=self.ttl_file, width=50).pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Button(demo_frame, text="🚀 Start Demo Processing", 
                  command=self.start_demo, style="Accent.TButton").pack(padx=10)
        ...
        # Status
        self.status = ttk.Label(self.root, text="Demo ready - Click 'Start Demo Processing' to begin")
    ...
    def start_demo(self):
        ...
        for i, concept in enumerate(concepts, 1):
            ...
            if comparison['discrepancies']:
                self.log(f"⚠️ Service discrepancies detected")
